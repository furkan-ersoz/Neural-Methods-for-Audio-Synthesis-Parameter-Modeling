...
// ─────────────────────────────────────────────────────────────────────────────
void SynthV1Editor::DropZone::paint(Graphics& g)
{
...
// ─────────────────────────────────────────────────────────────────────────────
void SynthV1Editor::paint(Graphics& g)
{
...
// ─────────────────────────────────────────────────────────────────────────────
bool SynthV1Editor::isInterestedInFileDrag(const StringArray& files)
{
    for (auto& f : files) {
...
void SynthV1Editor::filesDropped(const StringArray& files, int, int)
{
...
void SynthV1Editor::fileDragEnter(const StringArray&, int, int)
{
    dropZone_.isDragOver = true;
    dropZone_.repaint();
}

void SynthV1Editor::fileDragExit(const StringArray&)
{
    dropZone_.isDragOver = false;
    dropZone_.repaint();
}
...
// ─────────────────────────────────────────────────────────────────────────────
void SynthV1Editor::processFile(const File& file)
{
    if (!processor_.getInferenceEngine().isModelLoaded()) {
        setStatus("Model not loaded.", true);
        return;
    ...
    processor_.getInferenceEngine().runInference(
        ...
        [this](const std::vector<float>& params, const String& error) {
            ...
            for (int i = 0; i < static_cast<int>(params.size())
                            && i < static_cast<int>(SynthV1::PARAM_DESCRIPTORS.size()); ++i)
                msg += " " + String(SynthV1::PARAM_DESCRIPTORS[static_cast<size_t>(i)].name.data())
...
// ─────────────────────────────────────────────────────────────────────────────
void SynthV1Editor::setStatus(const String& msg, bool isError)
{
