...
// ─────────────────────────────────────────────────────────────────────────────
void SynthV2Editor::DropZone::paint(Graphics& g)
{
...
// ─────────────────────────────────────────────────────────────────────────────
SynthV2Editor::SynthV2Editor(SynthV2Processor& p)
...
// ─────────────────────────────────────────────────────────────────────────────
void SynthV2Editor::paint(Graphics& g)
{
...
// ─────────────────────────────────────────────────────────────────────────────
bool SynthV2Editor::isInterestedInFileDrag(const StringArray& files)
{
    for (auto& f : files) {
...
void SynthV2Editor::filesDropped(const StringArray& files, int, int)
{
...
void SynthV2Editor::fileDragEnter(const StringArray&, int, int)
{
    dropZone_.isDragOver = true;
    dropZone_.repaint();
}

void SynthV2Editor::fileDragExit(const StringArray&)
{
    dropZone_.isDragOver = false;
    dropZone_.repaint();
}
...
// ─────────────────────────────────────────────────────────────────────────────
void SynthV2Editor::processFile(const File& file)
{
    if (!processor_.getInferenceEngine().isModelLoaded()) {
        setStatus("Model not loaded.", true);
        return;
    ...
    processor_.getInferenceEngine().runInference(
        ...
        [this](const std::vector<float>& params, const String& error) {
            ...
            for (int i = 0; i < static_cast<int>(params.size())
                            && i < static_cast<int>(SynthV2::PARAM_DESCRIPTORS.size()); ++i)
                msg += " " + String(SynthV2::PARAM_DESCRIPTORS[static_cast<size_t>(i)].name.data())
...
// ─────────────────────────────────────────────────────────────────────────────
void SynthV2Editor::setStatus(const String& msg, bool isError)
{
