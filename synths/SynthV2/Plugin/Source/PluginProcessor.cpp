...
SynthV2Processor::createParameterLayout()
{
    ...
    for (const auto& desc : SynthV2::PARAM_DESCRIPTORS)
        params.push_back(std::make_unique<AudioParameterFloat>(
...
// ─────────────────────────────────────────────────────────────────────────────
SynthV2Processor::SynthV2Processor()
      ...
      apvts_(*this, nullptr, "STATE", createParameterLayout())
{
    ...
    if (modelFile.existsAsFile())
        ...
        Logger::writeToLog("SynthV2: ONNX model not found at: "
                           + modelFile.getFullPathName());
}

// ─────────────────────────────────────────────────────────────────────────────
juce::File SynthV2Processor::findModelFile() const
{
    const auto bundleDir = File::getSpecialLocation(
                               File::currentApplicationFile).getParentDirectory();

    const juce::String modelName = "SynthV2Model.onnx";
...
// ─────────────────────────────────────────────────────────────────────────────
void SynthV2Processor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
    for (auto& v : voices_)
...
// ─────────────────────────────────────────────────────────────────────────────
SynthV2::SynthParameters SynthV2Processor::paramsFromAPVTS() const
{
    SynthV2::SynthParameters p;
    ...
    vals.reserve(SynthV2::NUM_PARAMS);
    for (const auto& desc : SynthV2::PARAM_DESCRIPTORS)
...
// ─────────────────────────────────────────────────────────────────────────────
int SynthV2Processor::findFreeVoice() const
{
...
// ─────────────────────────────────────────────────────────────────────────────
void SynthV2Processor::processBlock(AudioBuffer<float>& buffer,
...
// ─────────────────────────────────────────────────────────────────────────────
void SynthV2Processor::applyInferenceResult(const std::vector<float>& params)
{
    for (size_t i = 0; i < SynthV2::PARAM_DESCRIPTORS.size() && i < params.size(); ++i) {
        if (auto* p = apvts_.getParameter(SynthV2::PARAM_DESCRIPTORS[i].name.data()))
            p->setValueNotifyingHost(params[i]);
    }
}

// ─────────────────────────────────────────────────────────────────────────────
AudioProcessorEditor* SynthV2Processor::createEditor()
{
    return new SynthV2Editor(*this);
}

AudioProcessor* JUCE_CALLTYPE createPluginFilter()
{
    return new SynthV2Processor();
}
