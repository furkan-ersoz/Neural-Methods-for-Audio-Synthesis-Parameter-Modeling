...
// ─────────────────────────────────────────────────────────────────────────────
SynthTemplate::SynthParameters SynthTemplateProcessor::paramsFromAPVTS() const
{
    ...
    vals.reserve(SynthTemplate::NUM_PARAMS);
    for (const auto& desc : SynthTemplate::PARAM_DESCRIPTORS)
...
// ─────────────────────────────────────────────────────────────────────────────
int SynthTemplateProcessor::findFreeVoice() const
{
    ...
    auto renderUntil = [&](int endSample) {
        ...
        for (auto& v : voices_) {
            ...
            v.engine.process(monoBuf.data(), segLen, v.noteOn,
                             SynthTemplate::midiToHz(v.midiNote));
...
{
    for (size_t i = 0; i < SynthTemplate::PARAM_DESCRIPTORS.size() && i < params.size(); ++i) {
        if (auto* p = apvts_.getParameter(SynthTemplate::PARAM_DESCRIPTORS[i].name.data()))
