#!/bin/bash
# experiment03.sh — osc_a sanity check (5k samples, pure regression)
# Amac: modelin ogrendigini dogrulamak. Calistirmak icin:
#   bash bashs/experiment03.sh
set -e
...
echo "=============================================="
echo "  Experiment03 — osc_a sanity check"
echo "  Root: $ROOT"
echo "=============================================="
...
if [ ! -f "$CLI_BIN" ]; then
    CLI_BIN=$(find "$ROOT/build" "$ROOT/cmake-build-debug" -name "DataGeneratorCLI" -type f 2>/dev/null | head -1)
fi
if [ -z "$CLI_BIN" ] || [ ! -f "$CLI_BIN" ]; then
    echo "ERROR: DataGeneratorCLI binary bulunamadi. Once build edin."
    exit 1
fi
echo "Binary: $CLI_BIN"
...
# En son olusturulan S03D* dataset'i bul
...
# config.yaml icindeki dataset path'i guncelle
sed -i.bak "s|path:.*PLACEHOLDER.*|path: \"datasets/$DS_NAME\"|" "$SYNTH_CONFIG"
...
# ── 2. Feature extraction + PNG ───────────────────────────────────────────────
echo ""
echo "--- [2/4] Feature extraction + spectrogram PNG ---"
cd "$NN_DIR"
$PYTHON feature_extraction_v2.py --config "$SYNTH_CONFIG" --save-png --num-workers 4

# ── 3. Training — osc_a only ──────────────────────────────────────────────────
echo ""
...
print(f"  Random baseline: {random_baseline:.6f}  (uniform [0,1] params icin)")
...
echo ""
echo "  Spectrogram PNG'leri: $ROOT/synths/SynthV3/datasets/$DS_NAME/spectrograms/"
echo "  Checkpoints: $EXP_DIR/checkpoints/osc_a/"
echo "  Log: $EXP_DIR/logs/osc_a/train_log.json"
echo ""
echo "=============================================="
echo "  Experiment03 tamamlandi: $(date)"
echo "=============================================="
