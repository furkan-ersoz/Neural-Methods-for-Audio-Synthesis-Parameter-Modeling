...
def extract_features(wav_path: Path, cfg: FeatureConfig) -> np.ndarray:
    ...
    Deger araligi: normalize=True ise z-score [mean=0, std=1].
    """
...
def _z_score(feat: np.ndarray) -> np.ndarray:
    """
    Per-feature (her frekans bini icin) z-score normalizasyonu.
    Sifir std durumunda bolme hatasini onler.
    """
    mean = feat.mean(axis=1, keepdims=True)   # (C, 1)
    std  = feat.std(axis=1,  keepdims=True)   # (C, 1)
    std  = np.where(std < 1e-8, 1.0, std)     # sifir std korumasi
